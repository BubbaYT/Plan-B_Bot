...
@bot.event
async def on_ready():
    await bot.change_presence(game=discord.Game(name=" mc.venompvp.org "
                                                        "With Bubba ;D"))
@bot.command()
...
async def send(ctx):
    ...
    if ctx.author.permissions_in(ctx.channel).manage_guild:
        await ctx.send("**Plan-B is Recruiting .apply to apply ;D @everyone**")
...
async def apply(context):
    """: Apply to join"""
    auth = context.author
    channel = bot.get_channel(502471914095116289)

    await context.send("``You have started a application``")
    await auth.send("You are applying to join **Plan-B**\nYou have 30 seconds to respond")
...
async def accept(ctx, user: discord.Member):
    ...
    if ctx.author.permissions_in(ctx.channel).manage_roles:
     await ctx.send(f"{user.mention} has been accepted to **___Plan-B___**")
     await user.send("Welcome to **Plan-B** Please dont mute the discord!!!")
     role1 = discord.utils.get(ctx.guild.roles, name = "Wall Checkers")
     role2 = discord.utils.get(ctx.guild.roles, name = "Recruit")
...
async def reject(ctx, user: discord.Member):
    """: Rejects a applicant"""
    if ctx.author.permissions_in(ctx.channel).manage_roles:
      await ctx.send(f"{user.mention} has been denied from **Plan-B**")
...
bot.run("NTI3MDQzMjY2MTI2ODA3MDYw.DwN_Qg.RCxS_BKWge_u8cjmYtqkAox7k9w")
